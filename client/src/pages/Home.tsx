...
export default function Home() {
  ...
  const bundleReadiness = useMemo(() => workspace.bundles.map(bundle => {
    ...
    const completed = links.filter(link => workspace.drafts.find(draft => draft.id === link.reelDraftId && draft.sourceCited && draft.limitationLinePresent && draft.notMedicalAdvice && draft.sourcePackStatus === "complete" && draft.healthRedFlagsCleared && draft.bgmStatus === "ready" && draft.voiceStatus === "ready")).length;
