...
import { Check, CircleAlert, Clock3, FileSearch, KeyRound, Layers3, LockKeyhole, Plus, Send, ShieldAlert, Sparkles, UploadCloud, Video, Volume2, X } from "lucide-react";
...
const initialForm: CandidateForm = { title: "", journal: "", doi: "", studyType: "Human cohort" };
...
export default function Home() {
  ...
  const bundleReadiness = useMemo(() => workspace.bundles.map(bundle => {
    ...
    const completed = links.filter(link => workspace.drafts.find(draft => draft.id === link.reelDraftId && draft.sourceCited && draft.limitationLinePresent && draft.notMedicalAdvice && draft.bgmStatus === "ready" && draft.voiceStatus === "ready")).length;
    ...
    <DashboardLayout>
      <div className="mx-auto max-w-[1540px] px-4 py-5 sm:px-7 sm:py-8 lg:px-10">
        ...
        <section id="daily-research" className="scroll-mt-6 border-t border-[#d9ddd6] pt-9">
          <SectionHeading eyebrow="01 · Daily research queue" title="Evidence before aesthetics." detail="Every candidate carries a source trail, a study-type label, and a screening decision before it can enter production." action={<Dialog open={captureOpen} onOpenChange={setCaptureOpen}><DialogTrigger asChild><Button className="rounded-xl bg-[#202633] px-4 hover:bg-[#303949]"><Plus className="mr-2 h-4 w-4" />Add candidate</Button></DialogTrigger><DialogContent className="max-w-xl"><DialogHeader><DialogTitle className="font-serif text-2xl">Add a study candidate</DialogTitle><DialogDescription>Duplicates are checked against the content log before the study enters the research queue.</DialogDescription></DialogHeader><form className="grid gap-4" onSubmit={event => { event.preventDefault(); addCandidate.mutate({ ...form, doi: form.doi || undefined }); }}><div className="grid gap-2"><Label htmlFor="study-title">Study title</Label><Input id="study-title" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="Enter the primary study title" required /></div><div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><div className="grid gap-2"><Label htmlFor="journal">Journal</Label><Input id="journal" value={form.journal} onChange={e => setForm({ ...form, journal: e.target.value })} placeholder="Journal name" required /></div><div className="grid gap-2"><Label>Study type</Label><Select value={form.studyType} onValueChange={(value: CandidateForm["studyType"]) => setForm({ ...form, studyType: value })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Human cohort">Human cohort</SelectItem><SelectItem value="Systematic review">Systematic review</SelectItem><SelectItem value="Replication study">Replication study</SelectItem><SelectItem value="Clinical trial">Clinical trial</SelectItem><SelectItem value="Preclinical model">Preclinical model</SelectItem></SelectContent></Select></div></div><div className="grid gap-2"><Label htmlFor="doi">DOI or PMID</Label><Input id="doi" value={form.doi} onChange={e => setForm({ ...form, doi: e.target.value })} placeholder="Optional source identifier" /></div><DialogFooter><Button type="submit" disabled={addCandidate.isPending}>{addCandidate.isPending ? "Screening…" : "Add to queue"}</Button></DialogFooter></form></DialogContent></Dialog>} />
          {isLoading ? <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{[1, 2, 3].map(item => <Skeleton key={item} className="h-44 rounded-2xl bg-[#e5e7e2]" />)}</div> : workspace.studies.length ? <div className="grid gap-3 xl:grid-cols-3">{workspace.studies.map(study => <article key={study.id} className="rounded-2xl border border-[#d9ddd6] bg-white p-5 shadow-[0_10px_28px_rgba(25,28,36,0.035)]"><div className="flex items-start justify-between gap-3"><StatusPill value={study.screeningStatus} />{study.isDuplicate && <Badge className="rounded-full bg-[#fff0ee] text-[10px] font-semibold text-[#a34839] hover:bg-[#fff0ee]">POSSIBLE REPEAT</Badge>}</div><h3 className="mt-4 font-serif text-xl leading-snug text-[#20232b]">{study.title}</h3><dl className="mt-5 grid grid-cols-2 gap-x-4 gap-y-3 border-t border-[#eceeea] pt-4 text-xs"><div><dt className="font-semibold uppercase tracking-[0.1em] text-[#8a9191]">Journal</dt><dd className="mt-1 text-[#4f5758]">{study.journal}</dd></div><div><dt className="font-semibold uppercase tracking-[0.1em] text-[#8a9191]">Study type</dt><dd className="mt-1 text-[#4f5758]">{study.studyType}</dd></div><div className="col-span-2"><dt className="font-semibold uppercase tracking-[0.1em] text-[#8a9191]">DOI / PMID</dt><dd className="mt-1 break-all font-mono text-[11px] text-[#4f5758]">{study.doi || study.pmid || "Identifier pending"}</dd></div></dl><div className="mt-4 rounded-xl bg-[#f6f7f4] px-3 py-2.5 text-xs leading-5 text-[#66706b]"><span className="font-semibold text-[#404845]">Screening note · </span>{study.screeningReason || "No screening note recorded."}</div></article>)}</div> : <EmptyPanel icon={FileSearch} title="The research queue is clear." detail="Add an evidence-screened study candidate to start a source trail, duplicate check, and editorial screen." />}
        </section>
        ...
        <section id="weekly-compilation" className="scroll-mt-6 border-t border-[#d9ddd6] pt-12">
          <SectionHeading eyebrow="03 · Weekly compilation queue" title="Seven days. One evidence trail." detail="A bundle is ready to compile only when every linked daily reel meets the same source, limitation, voice, and media requirements." />
