...
async function fetchRecentPubMedRecords(): Promise<PubMedRecord[]> {
  ...
  const searchResponse = await fetch(search, { headers: { accept: "application/json" } });
  ...
  const summaryResponse = await fetch(summary, { headers: { accept: "application/json" } });
...
export async function attachScheduleTaskUid(ownerId: number, jobType: AutomationJobType, taskUid: string) {
  const db = await getDb();
  if (!db) throw new Error("Database is unavailable");
...
async function runDailyResearch(job: typeof automationJobs.$inferSelect, triggerType: AutomationTrigger) {
  ...
  try {
    ...
    for (const record of records) {
      ...
      if (!title) continue;
      const duplicateCandidate = await db.select({ id: studyCandidates.id })
        .from(studyCandidates)
        .where(and(eq(studyCandidates.ownerId, job.ownerId), eq(studyCandidates.pmid, record.uid)))
        .limit(1);
...
async function runWeeklyCompilation(job: typeof automationJobs.$inferSelect, triggerType: AutomationTrigger) {
  ...
  try {
    ...
    const bundle = (await db.select().from(weeklyBundles)
      .where(and(eq(weeklyBundles.ownerId, job.ownerId), eq(weeklyBundles.weekStart, currentWeek)))
      .limit(1))[0];
    if (!bundle) {
...
export async function runAutomationJob(job: typeof automationJobs.$inferSelect, triggerType: AutomationTrigger) {
  if (!job.enabled) return { status: "blocked" as const, summary: "This automation job is paused.", candidateCount: 0 };
  return job.jobType === "daily_research" ? runDailyResearch(job, triggerType) : runWeeklyCompilation(job, triggerType);
}

export async function runAutomationJobByOwner(ownerId: number, jobType: AutomationJobType, triggerType: AutomationTrigger) {
  await ensureAutomationJobs(ownerId);
  const db = await getDb();
  if (!db) throw new Error("Database is unavailable");
  const job = (await db.select().from(automationJobs).where(and(eq(automationJobs.ownerId, ownerId), eq(automationJobs.jobType, jobType))).limit(1))[0];
  if (!job) throw new Error("Automation job not found");
...
export async function runAutomationJobByTaskUid(taskUid: string) {
  const db = await getDb();
  if (!db) throw new Error("Database is unavailable");
  const job = (await db.select().from(automationJobs).where(eq(automationJobs.scheduleCronTaskUid, taskUid)).limit(1))[0];
...
export async function getAutomationState(ownerId: number) {
  ...
  const runs = jobIds.length ? await db.select().from(automationRuns).where(inArray(automationRuns.jobId, jobIds)).orderBy(desc(automationRuns.startedAt)).limit(12) : [];
  return { jobs, runs };
}
