...
import {
  approveDraftForOwner,
  findLoggedTopic,
  getWorkspaceData,
  initializeCurrentWorkingDraft,
} from "./contentDb";
...
import {
  attachScheduleTaskUid,
  AUTOMATION_CRONS,
  ensureAutomationJobs,
  getAutomationState,
  runAutomationJobByOwner,
  ...
  workspace: router({
    ...
    addStudyCandidate: ownerProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const editorialFlags = buildEditorialFlags(
          category,
          input.sourceUrl,
          input.populationContext
        );
        const requiresOwnerReview = Boolean(
          priorLog || highScrutiny || editorialFlags.length
        );
        await db.insert(studyCandidates).values({
          ...
          crossValidationStatus: input.sourceUrl
            ? "needs_review"
            : "not_started",
          requiresOwnerReview,
          editorialFlags,
    ...
    approveForPublish: ownerProcedure
      ...
      .mutation(async ({ ctx, input }) =>
        approveDraftForOwner(ctx.user.id, input.reelDraftId)
      ),
    initializeCurrentDraft: ownerProcedure.mutation(async ({ ctx }) =>
      initializeCurrentWorkingDraft(ctx.user.id)
    ),
    getHinglishScriptTemplate: ownerProcedure
      .input(
        z.object({
          topic: z.string().min(4).max(180),
          contentCategory: z.enum([
            "neuroscience",
            "psychology",
            "diet",
      ...
      .query(({ input }) =>
        buildHinglishScriptTemplate(input.topic, input.contentCategory)
      ),
  }),
  automation: router({
    get: ownerProcedure.query(async ({ ctx }) =>
      getAutomationState(ctx.user.id)
    ),
    configureFreeSchedules: ownerProcedure.mutation(async ({ ctx }) => {
      ...
      for (const job of jobs) {
        if (job.scheduleCronTaskUid) {
          configured.push({
            jobType: job.jobType,
            taskUid: job.scheduleCronTaskUid,
            existing: true,
          });
          continue;
        }
    ...
    runNow: ownerProcedure
      .input(
        z.object({ jobType: z.enum(["daily_research", "weekly_compilation"]) })
      )
      .mutation(async ({ ctx, input }) =>
        runAutomationJobByOwner(ctx.user.id, input.jobType, "manual")
      ),
    setScheduleEnabled: ownerProcedure
      .input(
        z.object({
          jobType: z.enum(["daily_research", "weekly_compilation"]),
          enabled: z.boolean(),
        })
      )
      .mutation(async ({ ctx, input }) => {
        const sessionToken =
          parseCookie(ctx.req.headers.cookie ?? "")[COOKIE_NAME] ?? "";
        return setAutomationJobEnabled(
          ctx.user.id,
          input.jobType,
          input.enabled,
          sessionToken
