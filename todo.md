...
- [ ] Implement a bounded non-destructive repository maintenance workflow with non-secret state tracking, explicit validation steps, and no automatic social-publishing action.
- [ ] Validate the maintenance workflow locally and through public GitHub Actions, then classify all provider- or owner-dependent work without attempting security bypass.
- [ ] Configure a recurring maintenance schedule only when duplicate checks, runtime constraints, and manual-publishing safeguards are all verified.
- [ ] Add a daily scheduled GitHub Actions repository-health workflow that runs only safe validation commands and does not mutate Git history, connectors, secrets, or social platforms.
- [ ] Manually dispatch and verify the new daily maintenance workflow, then document its scope, schedule, and non-destructive boundaries.
